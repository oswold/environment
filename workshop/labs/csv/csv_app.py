...
st.set_page_config(page_title="Email to CSV", layout="wide")  #set the page width wider to accommodate columns

st.title("Email to CSV  Conversion")  #page title


st.subheader("Email") #subhead for this column

input_text = st.text_area("Input text", height=200, label_visibility="collapsed")

process_button = st.button("Convert", type="primary") #display a primary button
...
if process_button: #code in this if block will be run when the button is clicked
    with st.spinner("CSVing..."): #show a spinner while the code in this with block runs
